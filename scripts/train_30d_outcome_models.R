...
# Train 30-day landmark models for final loan outcome prediction.
# Models:
...
REPORT_PATH <- "docs/loan_outcome_30d_model_results.md"
...
df <- raw[raw$eligible_30d == "1", , drop = FALSE]
df$target_final_outcome <- factor(df$target_final_outcome, levels = label_levels)
df <- df[!is.na(df$target_final_outcome), , drop = FALSE]

w30_features <- grep("^w30d_", names(df), value = TRUE)
feature_cols <- c(static_features, w30_features)
...
write.csv(evals$multinom_validation$predictions, file.path(PRED_DIR, "loan_outcome_30d_multinom_validation_predictions.csv"), row.names = FALSE)
write.csv(evals$multinom_test$predictions, file.path(PRED_DIR, "loan_outcome_30d_multinom_test_predictions.csv"), row.names = FALSE)
write.csv(evals$random_forest_validation$predictions, file.path(PRED_DIR, "loan_outcome_30d_random_forest_validation_predictions.csv"), row.names = FALSE)
write.csv(evals$random_forest_test$predictions, file.path(PRED_DIR, "loan_outcome_30d_random_forest_test_predictions.csv"), row.names = FALSE)

saveRDS(
  list(model = multinom_model, preprocess = prep, feature_cols = feature_cols),
  file.path(MODEL_DIR, "loan_outcome_30d_multinom.rds")
)
saveRDS(
  list(model = rf_model, preprocess = prep, feature_cols = feature_cols),
  file.path(MODEL_DIR, "loan_outcome_30d_random_forest.rds")
)
...
write.csv(metrics_summary, file.path(PRED_DIR, "loan_outcome_30d_metrics.csv"), row.names = FALSE)
...
write.csv(rf_importance_df, file.path(PRED_DIR, "loan_outcome_30d_random_forest_feature_importance.csv"), row.names = FALSE)
